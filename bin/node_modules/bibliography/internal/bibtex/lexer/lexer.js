...
class Lexer {
  ...
  readNextToken() {
    ...
    if (Lexer.isWhiteSpace(c)) return this.getWhiteSpace(c);else if (Lexer.isSpecialChar(c)) {
      this.pos++;
      if (c == '@') {
        ...
        switch (type) {
          ...
          case 'preamble':
          case 'comment':
            return new _Token2.default('@' + type, type); // TODO maybe use symbols to be less memory intensive?
          default:
            return new _Token2.default('@bib', type);
        }
      }
      return c;
      ...
      for (var n = pos2; n < this.len; n++) {
        ...
        if (Lexer.isNum(charAt)) nums.push(charAt);else break;
      }
      const numString = nums.join("");
      if (numString[0] == '0') // If it starts with 0, return as a string
        return new _Token2.default(NUMBER, numString);

      const number = Number.parseInt(numString);
      if (Number.isFinite(number)) return number;else return new _Token2.default(NUMBER, numString);
    } else {
      // id
      let chars = [];
      const pos2 = this.pos;
      ...
      return new _Token2.default(ID, chars.join("").trim());
    }
  }
  ...
  getWhiteSpace() {
    ...
    return new _Token2.default(whitespace, chars.join(""));
  }
