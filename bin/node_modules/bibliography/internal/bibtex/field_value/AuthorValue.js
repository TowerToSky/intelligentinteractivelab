...
function splitOnAnd(authorTokens) {
  return authorTokens.reduce((prev, curr) => {
    //console.log(curr);
    if (curr.length == 1 && curr[0] == 'and') prev.push([]);else prev[prev.length - 1].push(curr);
    return prev;
  }, [[]]);
}
...
function firstVonLast(authorTokens) {
  ...
  for (let i = 0; i < authorTokens.length - 1; i++) {
    // -1 because last word must be lastName
    //console.log("STARLOW", (authorTokens[i]));
    //console.log("STARLOW", startsWithLowerCase(authorTokens[i]));
    if ((0, _utils.startsWithLowerCase)(authorTokens[i])) {
      if (vonStartInclusive < 0) vonStartInclusive = i;
      vonEndExclusive = i + 1;
    }
  }
  if (vonStartInclusive > 0) firstNameEndExclusive = vonStartInclusive;else firstNameEndExclusive = authorTokens.length - 1;
  ...
  return new _PersonName2.default(firstName, von, lastName, []);
}

function vonLastFirst(authorTokens) {
  let commaPos = -1;
  for (let i = 0; i < authorTokens.length; i++) if (authorTokens[i].type == ',') {
    commaPos = i;
    break;
  }
  let vonStartInclusive = -1;
  ...
  for (let i = 0; i < commaPos; i++) if ((0, _utils.startsWithLowerCase)(authorTokens[i])) {
    if (vonStartInclusive < 0) vonStartInclusive = i;
    vonEndExclusive = i + 1;
  }
...
function vonLastJrFirst(authorTokens) {
  ...
  let commaPos2 = -1;
  for (let i = commaPos + 1; i < authorTokens.length; i++) if (authorTokens[i].type == ',') {
    commaPos2 = i;
    break;
  }
  let vonStartInclusive = -1;
  ...
  return new _PersonName2.default(firstName, von, lastName, jr);
}
...
class AuthorValue extends _StringValue2.default {
  constructor(raw) {
    super(raw);
