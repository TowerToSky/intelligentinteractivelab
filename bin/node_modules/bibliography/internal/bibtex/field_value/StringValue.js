...
var _specialCharsHandlers = require('./specialCharsHandlers');
...
function flatten(obj) {
  if (typeof obj == 'object' && (obj.type == 'quotedstring' || obj.type == 'quotedstringwrapper' || obj.type == 'bracedstringwrapper')) return flatten(obj.data);else if (typeof obj == 'object' && obj.type == 'ws') return obj;else if (obj._raw) return flatten(obj._raw);else if (typeof obj == 'object' && obj.string) return obj.string;else if (obj.constructor == Number) return obj + "";else if (typeof obj == 'string') return obj;else if (obj.constructor == Array) {
    let tokens = [];
    obj.forEach(o => {
      const conc = flatten(o);
      //console.log(JSON.stringify(o));
      //console.log(JSON.stringify(conc));
      if (conc.constructor == Array) tokens = tokens.concat(conc);else tokens.push(conc);
    });
    return tokens;
...
function toWords(wordParts, retainWhitespace) {
  return wordParts.reduce((prev, current) => {
    if (current.type == 'ws') {
      if (retainWhitespace) prev[prev.length - 1].push(current);else prev.push([]);
    ...
    } else if (typeof current == 'string') prev[prev.length - 1].push(current);else throw new Error("! toWords error: " + JSON.stringify(current));
    return prev;
  }, [[]]);
}
function concatStrings(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    const obj = array[i];
    if (obj == ',') words.push({ type: ',' });else if (typeof obj == 'string') {
      if (typeof words[words.length - 1] == 'string') words[words.length - 1] = words[words.length - 1] + obj;else words.push(obj);
    } else if (obj.type == 'ws') words.push(obj);else if (obj.type == 'braced') words.push({
      type: obj.type,
  ...
  return words.map(word => {
    if (word.constructor == Array) {
      return word.map(wordSegment => {
        if (wordSegment.type == 'braced' && wordSegment.data[0] && wordSegment.data[0].constructor == Array && wordSegment.data[0].length == 1 && typeof wordSegment.data[0][0] == 'string' && wordSegment.data[0][0].charAt(0) == '\\') {
          const escapeString = wordSegment.data[0][0];
          const specialChar = _specialCharsHandlers.specialChars[escapeString.substring(1)];
          if (specialChar) return {
            type: 'specialChar',
            data: wordSegment,
            unicode: specialChar(specialChar)
          };
          const diacriticHandler = _specialCharsHandlers.diacritics[escapeString.charAt(1)];
...
class StringValue {
  constructor(strRaw) {
    ...
    if (strRaw.type != 'quotedstringwrapper' && strRaw.type != 'bracedstringwrapper') throw new Error("Did not expect object to instantiate StringValue: " + JSON.stringify(strRaw));
  ...
  static computeUnicodeString(braceDepth, obj) {
    ...
    if (typeof obj == 'string') return obj;else if (obj.constructor == Array) return obj.map(o => {
    ...
    }).join('');else if (obj.toUnicode) {
      //console.log("toUnicode", JSON.stringify(obj));
      //console.log("toUnicode", (obj.toUnicode()));
      return obj.toUnicode();
    } else if (typeof obj == 'object' && (obj.type == 'number' || obj.type == 'id' || obj.type == 'ws')) return StringValue.computeUnicodeString(0, obj.string);else if (typeof obj == 'object' && obj.unicode) return obj.unicode;else if (obj.constructor === Number) return obj + "";else if (typeof obj == 'object' && obj.type == 'quotedstring') return StringValue.computeUnicodeString(braceDepth, obj.data);else if (typeof obj == 'object' && (obj.type == 'quotedstringwrapper' || obj.type == 'bracedstringwrapper')) return StringValue.computeUnicodeString(0, obj.data);
    /**
     * A special character is a
     * part of a field starting with a left brace being at brace depth 0 immediately followed with a backslash,
     * and ending with the corresponding right brace. For instance, in the above example, there is no special
  ...
  static joinSimpleString(data) {
    if (typeof data == 'string') return data;else if (data.constructor == Array) {
      const str = [];
      for (let i = 0; i < data.length; i++) {
        str.push(StringValue.joinSimpleString(data[i]));
      }
      return str.join('');
    } else if (typeof data.string == 'string') return data.string;else if (data.type == 'braced') return StringValue.joinSimpleString(data.data);else if (typeof data.data == 'string') return data.string;else throw new Error("Could not read escaped string value " + JSON.stringify(data));
  }

  static resolveStrings(keyvals) {
    const refs = {};
    for (let key in keyvals) if (keyvals.hasOwnProperty(key) && !refs[key]) refs[key] = StringValue.resolveStringDeclarations(_immutable.Set.of(), keyvals[key], refs, keyvals);
    return refs;
  }
    ...
    if (wrapper.type == 'quotedstringwrapper') {
      return new StringValue({
        ...
        data: wrapper.data.map(strObj => {
          ...
          } else if (strObj._raw) return strObj;else return strObj;
        })
      });
    } else if (wrapper.type == 'bracedstringwrapper') return new StringValue(wrapper);else throw new Error("Unexpected object to resolve: " + JSON.stringify(wrapper));
  }
